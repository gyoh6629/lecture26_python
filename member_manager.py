class Member:
    def __init__(self, num, id, pw, name, phone, add):
        ...
        self.__add = add
    def __str__(self):
        return f'회원번호\t아이디\t\t비밀번호\t이름\t\t전화번호\t주소\n{self.__num}\t\t{self.__id}\t\t{self.__pw}\t\t{self.__name}\t\t{self.__phone}\t\t{self.__add}'
...
while True:
    ...
    if select == 1:
        ...
        print('회원목록')
        print('============')
        for member_list in ms.list_member():
            print(member_list)
    elif select == 3:
        print('================')
        print('회원상세정보')
        print('================')
        num = input('회원번호 입력 : ')
        print(ms.list_member_details(num))
    elif select == 4:
        print('================')
        print('회원정보수정')
        print('================')
